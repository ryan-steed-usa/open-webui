...
const usageStatsCache = new Map<string, Record<string, any>>();
...
export const usageStatsToHumanReadable = (
	...
	const cacheKey = `usage-${JSON.stringify(usage)}`;
	if (usageStatsCache.has(cacheKey)) return usageStatsCache.get(cacheKey);
	...
	usageStatsCache.set(cacheKey, results);
	return results;
};
